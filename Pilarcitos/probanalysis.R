...
path <- 'C:/repositories/codeRepo/UCBcode-R/trunk/Pilarcitos'
setwd(path)
...
# plot
freqplot <- ggplot(fogdat[fogdat[['condition']]=='f',], aes(location, fill=hour)) + 
  geom_bar() + facet_grid(year~month)
relfreqplot <- ggplot(fogdat[fogdat[['condition']]=='f',], aes(location, fill=condition)) + 
