...
# psi = initial guess for breakpoint
	...
	# segmented fit
	segfit <- segmented.lm(linfit, seg.Z=as.formula(paste("~", pv)), psi=600)
	bp <- segfit$psi[2]
	# redo segmented fit
...
# id = optional column name to use as point labels
	...
	for(i in seq(along=models)){
		...
		cnf <- geom_ribbon(data=new.data,
		                   aes(y=pred.full, ymin=ymin, ymax=ymax),
						   alpha=0.2)
		eqn <- geom_text(label=formulas[[i]], size=2,
				     x=psi, y=0.99*max(df[[rv]]))
		rval <- geom_text(label=paste('r-squared ~', 
									  format(summary(models[[i]])$r.squared, 
													 digits=3)),
					x=psi, y=0.98*max(df[[rv]]), size=2)
